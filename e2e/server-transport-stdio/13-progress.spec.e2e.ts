/**
 * progress notification tests for the coremcp stdio server transport via StdioConnector
 *
 * validates progress reporting during long-running operations, progress with
 * known total, and progress without total using the StdioConnector against
 * our coremcp stdio server.
...
describe('server-transport-stdio / 13-progress', () => {
  ...
  describe('progress with known total', () => {
    it('should send progress notifications during long operations [PROGRESS-001]', async () => {
      ...
      const progressCtx = createServerStdioClientContext({
  ...
  describe('progress resets timeout', () => {
    it.todo(
      'should reset client-side timeout on progress notifications [PROGRESS-003]' +
        ' — requires per-request timeout configuration in StdioConnector' +
      ...
      async () => {
         ...
         *
         * pseudo-code:
         * 1. Create a StdioConnector with a short per-request timeout (e.g., 3s)
