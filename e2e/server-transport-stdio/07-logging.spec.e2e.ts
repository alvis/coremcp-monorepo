/**
 * E2E tests for logging via stdio transport using StdioConnector
 *
...
describe('server-transport-stdio / logging', () => {
  ...
  it('should set log level to warning [LOGGING-001]', async () => {
     ...
     */
    await expect(
      ctx.connector.setLogLevel('warning'),
  ...
  describe('all 8 standard log levels [LOGGING-003]', () => {
    it('should emit notifications for all 8 RFC 5424 log levels [LOGGING-003]', async () => {
      ...
      const levelsCtx = createServerStdioClientContext({
        onNotification: async (notification) => {
          notifications.push(notification);
        },
      });
