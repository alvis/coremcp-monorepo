...
/** actual port assigned to the running auth server (set after listen) */
let activePort = 0;
 ...
 * @param port port to listen on (defaults to 0 for dynamic allocation)
 * @returns promise that resolves with the actual port when the server is listening
 */
export async function startAuthServer(
  ...
  httpServer = createServer((req, res) => {
    ...
    if (pathname === '/.well-known/oauth-authorization-server') {
      handleMetadata(req, res, activePort);
  ...
  return new Promise((resolve) => {
    httpServer!.listen(requestedPort, '127.0.0.1', () => {
      activePort = (httpServer!.address() as AddressInfo).port;
      resolve(activePort);
    });
  });
}
