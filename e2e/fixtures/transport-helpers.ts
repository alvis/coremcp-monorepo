...
export interface ClientHttpContext {
  /** spawned HTTP server process, undefined when using a shared server */
  serverProcess: ChildProcess | undefined;
  /** configured HttpMcpConnector instance */
  connector: HttpMcpConnector;
  /** base URL of the server */
  baseUrl: string;
  /** tears down the context by disconnecting and killing the server if spawned */
...
export interface ServerHttpClientContextOptions {
  ...
  /** port of an existing server to connect to instead of spawning a new one */
  port?: number;
}
...
export async function createClientHttpContext(
  ...
  // when a port is provided, connect to the existing shared server
  // otherwise, spawn a dedicated server-everything process
  const serverInfo = providedPort
    ? { process: undefined, port: providedPort }
    : await spawnHttpServer();
...
export async function createServerHttpClientContext(
  ...
  // when a port is provided, connect to the existing shared server
  // otherwise, spawn a dedicated test server process
