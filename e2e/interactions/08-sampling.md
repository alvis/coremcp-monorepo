...
1. **Server -> Client**: `sampling/createMessage` with `_meta` containing related-task metadata
   ```json
   {
     ...
     "params": {
       ...
       "maxTokens": 100,
       "_meta": {
         "io.modelcontextprotocol/related-task": { "taskId": "task-abc-123" }
       }
     }
   }
   ```
...
- The value SHOULD be an object with a `taskId` property containing the task ID string of the originating task
