/**
 * sampling tests for the coremcp HTTP client connector against server-everything
 *
 * validates that our HttpMcpConnector correctly handles server-initiated
 * sampling/createMessage requests. server-everything's trigger-sampling-request tool
...
import {
  clientHttpTest,
  clientHttpTestWithFactory,
} from '../fixtures/http-test-fixtures';
...
describe('client-connector-http / 08-sampling', () => {
  describe('sampling without handler', () => {
    clientHttpTest(
      'should reject sampling request when no sampling handler is configured [SAMPLING-001]',
      async ({ connector }) => {
        // SPEC ALIGNMENT: PASS
        /**
         ...
         */
        // the connector created by clientHttpTest does not have
        // sampling capability enabled, so calling trigger-sampling-request on server-everything
        // should result in an error because the client cannot handle
        // sampling/createMessage requests
        await expect(
          connector.callTool('trigger-sampling-request', {
  ...
  describe('trigger-sampling-request tool availability', () => {
    clientHttpTestWithFactory(
      'should list trigger-sampling-request as an available tool [SAMPLING-001]',
      async ({ createContext }) => {
        // SPEC ALIGNMENT: PASS
        /**
         * verifies that the server-everything exposes a trigger-sampling-request tool which triggers
        ...
        const tools = await context.connector.listTools();
        const toolNames = tools.map((t) => t.name);

        expect(toolNames).toContain('trigger-sampling-request');
      },
    );

    clientHttpTestWithFactory(
      'should include trigger-sampling-request tool with proper input schema [SAMPLING-001]',
      async ({ createContext }) => {
        // SPEC ALIGNMENT: PASS
        /**
         * verifies the trigger-sampling-request tool has a description and inputSchema, confirming it
        ...
        const tools = await context.connector.listTools();
        const sampleTool = tools.find(
          (t) => t.name === 'trigger-sampling-request',
        );
  ...
  describe('sampling with handler', () => {
    clientHttpTestWithFactory(
      'should handle sampling/createMessage and return result [SAMPLING-001]',
      async ({ createContext }) => {
        // SPEC ALIGNMENT: PASS
        /**
        ...
        // server-everything's trigger-sampling-request tool triggers server->client
        ...
        if (!toolNames.includes('trigger-sampling-request')) {
          return;
        }

        const result = await context.connector.callTool(
  ...
  describe('sampling with model preferences', () => {
    clientHttpTest.todo(
      ...
      async () => {
         ...
         * 3. call trigger-sampling-request tool with modelPreferences (hints, costPriority, intelligencePriority)
