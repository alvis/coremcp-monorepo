...
describe('client-connector-http / 08-sampling', () => {
  describe('sampling without handler', () => {
    ...
    it('should reject sampling request when no sampling handler is configured [SAMPLING-001]', async () => {
       ...
       */
      // the connector created by createClientHttpContext does not have
  ...
  describe('task-augmented sampling', () => {
    it.todo(
      ...
      async () => {
         ...
         * 4. verify the server receives a task object with id, status, and result fields 5. verify the task can be polled/resolved to get the final CreateMessageResult
         * @see https://modelcontextprotocol.io/specification/2025-11-25/client/sampling#creating-messages
