...
describe('client-connector-stdio / 12-cancellation', () => {
  ...
  describe('cancel in-flight request', () => {
    it.todo(
      ...
      async () => {
         ...
         * but the current StdioConnector API does not expose the requestId or an AbortController-
         * style hook needed to emit that notification while callTool is pending.
         *
         * pseudo-code:
         * 1. create a stdio client context and start a long-running tool call without awaiting it
  ...
  describe('cancellation notification format', () => {
    it.todo(
      ...
      async () => {
         ...
         *
         * pseudo-code:
         * 1. create a stdio client context with sampling or elicitation capability enabled
  ...
  describe('cancel after completion', () => {
    it('should handle completed request without errors when late cancel would arrive [CANCEL-003]', async () => {
      // SPEC ALIGNMENT: PASS
      /**
       * verifies a late cancellation for an already-completed request is ignored.
       * per spec, receivers MAY ignore cancellation if processing has completed.
       * @see https://modelcontextprotocol.io/specification/2025-11-25/basic/utilities/cancellation#behavior-requirements
       * @see https://github.com/modelcontextprotocol/typescript-sdk/blob/ccb78f20438a853f0205b8ed7f642af7780208e6/packages/core/src/shared/protocol.ts#L635-L642 (_oncancel no-op after cleanup)
       */
