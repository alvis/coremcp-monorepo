...
describe('client-connector-stdio / 08-sampling', () => {
  describe('sampling without handler', () => {
    ...
    it('should reject sampling request when no sampling handler is configured [SAMPLING-001]', async () => {
      ...
      await expect(
        ctx.connector.callTool('trigger-sampling-request', {
  ...
  describe('trigger-sampling-request tool availability', () => {
    ...
    it('should list trigger-sampling-request as an available tool [SAMPLING-001]', async () => {
      // SPEC ALIGNMENT: PASS
      /**
    ...
    it('should include trigger-sampling-request tool with proper input schema [SAMPLING-001]', async () => {
      // SPEC ALIGNMENT: PASS
      /**
       ...
       */
      const tools = await ctx.connector.listTools();
      const sampleTool = tools.find((t) => t.name === 'trigger-sampling-request');
  ...
  describe('sampling with handler', () => {
    ...
    it('should handle sampling/createMessage and return result [SAMPLING-001]', async () => {
      ...
      const result = (await ctx.connector.callTool('trigger-sampling-request', {
        prompt: 'Test sampling',
        maxTokens: 10,
      })) as CallToolResult;
