...
import { serverHttpTestWithFactory } from '../fixtures/http-test-fixtures';
...
describe('basic sampling request', () => {
  serverHttpTestWithFactory(
    'should send sampling/createMessage to client and receive result [SAMPLING-001]',
    async ({ createContext }) => {
      // SPEC ALIGNMENT: PASS
      /**
      ...
      const result = await context.connector.callTool('trigger-sampling', {
        prompt: 'What is 2+2?',
        maxTokens: 100,
      });
      ...
      const jsonMatch = responseText.match(/Sampling result received: (.+)/);
...
describe('sampling with model preferences', () => {
  serverHttpTestWithFactory(
    'should send sampling request with modelPreferences [SAMPLING-002]',
    async ({ createContext }) => {
      // SPEC ALIGNMENT: PASS
      /**
       ...
       */
      const captured = {
        params: null as CreateMessageRequest['params'] | null,
      };
      ...
      const context = await createContext({
        ...
        onRequest: async (request) => {
          if (request.method === 'sampling/createMessage') {
            captured.params = (request as CreateMessageRequest).params;
      ...
      expect(captured.params!.modelPreferences).toBeDefined();
      expect(captured.params!.modelPreferences).toEqual(
        expect.objectContaining({
          hints: [{ name: 'claude-3' }],
          costPriority: 0.5,
          intelligencePriority: 0.8,
...
describe('sampling with tools', () => {
  serverHttpTestWithFactory(
    'should send sampling request with tools and toolChoice [SAMPLING-003]',
    async ({ createContext }) => {
      // SPEC ALIGNMENT: PASS
      /**
       * verifies the server tlChoice in a sampling/createMessage
      ...
      expect(captured.params).not.toBeNull();
      expect(captured.params!.tools).toBeDefined();
      ...
      expect(captured.params!.toolChoice).toBeDefined();
      expect(captured.params!.toolChoice).toEqual({ mode: 'auto' });
    },
  );
});
...
describe('task-augmented sampling', () => {
  serverHttpTestWithFactory(
    'should send sampling request with task metadata [SAMPLING-005]',
    async ({ createContext }) => {
      // SPEC ALIGNMENT: PASS
      /**
      ...
      const rawSession = await createRawHttpSession(context.mcpEndpoint);
      ...
      try {
        ...
        expect(meta!['io.modelcontextprotocol/related-task']).toBeDefined();
...
describe('sampling parameter handling', () => {
  serverHttpTestWithFactory(
    'should send sampling request with custom prompt text [SAMPLING-001]',
    async ({ createContext }) => {
      // SPEC ALIGNMENT: PASS
      /**
  ...
  serverHttpTestWithFactory(
    'should send sampling request with default maxTokens [SAMPLING-001]',
    async ({ createContext }) => {
      // SPEC ALIGNMENT: PASS
      /**
