/**
 * E2E tests for tools via HTTP transport using HttpMcpConnector
 *
...
serverHttpTest(
  'should list all tools with metadata [TOOL-001]',
  async ({ connector }) => {
    // SPEC ALIGNMENT: PASS
    /**
...
serverHttpTest('should call echo tool [TOOL-002]', async ({ connector }) => {
  // SPEC ALIGNMENT: PASS
  /**
...
serverHttpTest(
  'should call add tool with numeric args [TOOL-002]',
  async ({ connector }) => {
    // SPEC ALIGNMENT: PASS
    /**
...
serverHttpTest(
  'should call get-image and validate binary [TOOL-003]',
  async ({ connector }) => {
    // SPEC ALIGNMENT: PASS
    /**
...
serverHttpTest(
  'should call structured-output tool and return structuredContent [TOOL-004]',
  async ({ connector }) => {
    // SPEC ALIGNMENT: PASS
    /**
    ...
    const result = await connector.callTool('structured-output', {
      itemCount: 2,
    });
...
serverHttpTest(
  'should handle unknown tool error [TOOL-002]',
  async ({ connector }) => {
    // SPEC ALIGNMENT: PASS
    /**
    ...
    try {
      await connector.callTool('nonexistent-tool');
...
describe('tools/list pagination', () => {
  serverHttpTest(
    'should paginate tools with cursor-based navigation [TOOL-001]',
    async ({ connector }) => {
      // SPEC ALIGNMENT: PASS
      /**
...
describe('tools/list_changed notification', () => {
  serverHttpTest(
    'should receive list_changed notification when triggered [TOOL-006]',
    async () => {
      // SPEC ALIGNMENT: PASS
      /**
...
describe('tool isError flag', () => {
  serverHttpTest(
    'should throw ToolError for failing tool in sync mode [TOOL-003]',
    async ({ connector }) => {
      // SPEC ALIGNMENT: PASS
      /**
      ...
      try {
        await connector.callTool('task-failing', {
