...
describe('form mode elicitation', () => {
  serverHttpTestWithFactory(
    'should send elicitation/create to client and receive result [ELICITATION-001]',
    async ({ createContext }) => {
      // SPEC ALIGNMENT: PASS
      /**
      ...
      const result = await context.connector.callTool('trigger-elicitation', {
...
describe('URL mode elicitation', () => {
  serverHttpTestWithFactory(
    'should send URL mode elicitation/create to client [ELICITATION-002]',
    async ({ createContext }) => {
      // SPEC ALIGNMENT: PASS
      /**
...
describe('elicitation complete notification', () => {
  serverHttpTestWithFactory(
    'should emit notifications/elicitation/complete after URL mode elicitation [ELICITATION-003]',
    async ({ createContext }) => {
      // SPEC ALIGNMENT: PASS
      /**
      ...
      const notifyContext = await createContext({
        capabilities: {
          roots: { listChanged: true },
          elicitation: {},
        },
      ...
      const result = await notifyContext.connector.callTool(
...
describe('user decline/cancel', () => {
  serverHttpTestWithFactory(
    'should handle user decline action [ELICITATION-004]',
    async ({ createContext }) => {
      // SPEC ALIGNMENT: PASS
      /**
  ...
  serverHttpTestWithFactory(
    'should handle user cancel action [ELICITATION-004]',
    async ({ createContext }) => {
      // SPEC ALIGNMENT: PASS
      /**
...
describe('task-augmented elicitation', () => {
  serverHttpTestWithFactory(
    'should include related task metadata in elicitation during task execution [ELICITATION-006]',
    async ({ createContext }) => {
      // SPEC ALIGNMENT: PASS
      /**
...
describe('elicitation with different messages', () => {
  serverHttpTestWithFactory(
    'should send elicitation with custom message text [ELICITATION-001]',
    async ({ createContext }) => {
      // SPEC ALIGNMENT: PASS
      /**
  ...
  serverHttpTestWithFactory(
    'should send elicitation with default message [ELICITATION-001]',
    async ({ createContext }) => {
      // SPEC ALIGNMENT: PASS
      /**
