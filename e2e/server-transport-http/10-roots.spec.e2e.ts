...
describe('server-transport-http / 10-roots', () => {
  ...
  describe('list roots', () => {
    it('should send roots/list to client and receive roots result [ROOTS-001]', async () => {
      ...
      const result = await ctx.connector.callTool('trigger-roots-list', {});
  ...
  describe('empty roots list', () => {
    let emptyCtx: ServerHttpClientContext;

    beforeAll(async () => {
      emptyCtx = await createServerHttpClientContext({
        capabilities: { roots: { listChanged: true } },
        onRequest: async (request) => {
          if (request.method === 'roots/list') {
            return {
