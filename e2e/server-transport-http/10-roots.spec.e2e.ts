/**
 * roots tests for the coremcp HTTP server transport via HttpMcpConnector
 *
 * validates server-initiated roots/list requests triggered via
 * the trigger-roots-list tool. the connector acts as the client,
 * responding to roots/list requests from our server via onRequest handler.
...
describe('list roots', () => {
  serverHttpTestWithFactory(
    'should send roots/list to client and receive roots result [ROOTS-001]',
    async ({ createContext }) => {
      /* SPEC ALIGNMENT: PASS
      ...
      // trigger-roots-list calls session.reply with roots/list
      // the connector should handle the request via onRequest
      const result = await context.connector.callTool('trigger-roots-list', {});
  ...
  serverHttpTestWithFactory(
    '* per speceive a valid roots structure from client [ROOTS-001]',
    async ({ createContext }) => {
      // SPEC ALIGNMENT: PASS
      /**
      ...
      // the response contains the JSON stringified roots result
...
describe('roots list changed notification', () => {
  serverHttpTestWithFactory(
    'should send notifications/roots/list_changed to server [ROOTS-002]',
    async ({ createContext }) => {
      // SPEC ALIGNMENT: PASS
      /**
      ...
      await expect(
        context.connector.sendNotification('notifications/roots/list_changed'),
      ).resolves.toBeUndefined();
    },
  );

  serverHttpTestWithFactory(
    'should complete end-to-end roots flow after list_changed notification [ROOTS-002]',
    async ({ createContext }) => {
      // SPEC ALIGNMENT: PASS
      /**
      ...
      await context.connector.sendNotification(
      ...
      // verify the server received the roots from the client's onRequest handler
      expect(toolResult.content[0]).toEqual(
        expect.objectContaining({
          type: 'text',
          text: expect.stringContaining('Roots received'),
...
describe('empty roots list', () => {
  serverHttpTestWithFactory(
    'should handle empty roots list from client [ROOTS-003]',
    async ({ createContext }) => {
      // SPEC ALIGNMENT: PASS
      /**
