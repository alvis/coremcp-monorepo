...
describe('invalid JSON-RPC message', () => {
  serverHttpTest(
    'should return JSON-RPC parse error (-32700) for malformed JSON [ERROR-001]',
    async ({ mcpEndpoint }) => {
      // SPEC ALIGNMENT: PASS
      /**
  ...
  serverHttpTest(
    'should return error for missing jsonrpc field via raw HTTP [ERROR-001]',
    async ({ mcpEndpoint }) => {
      // SPEC ALIGNMENT: PASS
      /**
...
describe('method not found', () => {
  serverHttpTest(
    'should return JSON-RPC method not found error (-32601) for unknown method [ERROR-002]',
    async ({ connector }) => {
      // SPEC ALIGNMENT: PASS
      /**
  ...
  serverHttpTest(
    'should return JSON-RPC method not found error (-32601) for typo in method name [ERROR-002]',
    async ({ connector }) => {
      // SPEC ALIGNMENT: PASS
      /**
...
describe('invalid params', () => {
  serverHttpTest(
    'should return JSON-RPC invalid params error (-32602) for prompts/get with wrong field [ERROR-003]',
    async ({ connector }) => {
      // SPEC ALIGNMENT: PASS
      /**
...
describe('resource not found', () => {
  serverHttpTest(
    'should return error for nonexistent resource URI [ERROR-005]',
    async ({ connector }) => {
      // SPEC ALIGNMENT: PASS
      /**
       ...
       */
      try {
        await connector.readResource('test://nonexistent/resource');
        expect.unreachable('should have thrown');
      } catch (error: unknown) {
        expect(error).toBeInstanceOf(JsonRpcError);
...
describe('tool not found', () => {
  serverHttpTest(
    'should return error for nonexistent tool [ERROR-006]',
    async ({ connector }) => {
      // SPEC ALIGNMENT: PASS
      /**
  ...
  serverHttpTest(
    'should maintain connection after tool not found error [ERROR-006]',
    async ({ connector }) => {
      // SPEC ALIGNMENT: PASS
      /**
...
describe('connection recovery after errors', () => {
  serverHttpTest(
    'should handle request to server and recover after errors [ERROR-005]',
    async ({ connector }) => {
      // SPEC ALIGNMENT: PASS
      /**
  ...
  serverHttpTest(
    'should handle multiple consecutive errors without degradation [ERROR-005]',
    async ({ connector }) => {
      // SPEC ALIGNMENT: PASS
      /**
      ...
      await expect(connector.callTool('badTool1')).rejects.toThrow();
      await expect(connector.callTool('badTool2')).rejects.toThrow();
      await expect(
        connector.readResource('test://bad/resource'),
      ).rejects.toThrow();
...
describe('internal server error', () => {
  serverHttpTest(
    'should return -32603 internal error for trigger-internal-error tool [ERROR-004]',
    async ({ connector }) => {
      // SPEC ALIGNMENT: PASS
      /**
       ...
       */
      try {
        await connector.callTool('trigger-internal-error');
        expect.unreachable('should have thrown');
      } catch (error: unknown) {
        expect(error).toBeInstanceOf(JsonRpcError);
...
describe('bad protocol version', () => {
  serverHttpTest(
    'should reject initialize with unsupported protocol version [ERROR-007]',
    async ({ mcpEndpoint }) => {
      // SPEC ALIGNMENT: PASS
      /**
...
describe('server error responses', () => {
  serverHttpTest(
    'should handle raw HTTP error for disconnected / invalid session [ERROR-005]',
    async ({ mcpEndpoint }) => {
      // SPEC ALIGNMENT: PASS
      /**
...
describe('capability mismatch', () => {
  serverHttpTest(
    'should return error when calling trigger-roots-list without roots capability [ERROR-008]',
    async () => {
      // SPEC ALIGNMENT: PASS
      /**
      ...
      const noRootsContext = await createServerHttpClientContext({
        port: inject('coreMcpPort'),
        capabilities: {},
      });

      try {
        await noRootsContext.connector.connect();
        ...
        await expect(
          noRootsContext.connector.callTool('trigger-roots-list'),
        ).rejects.toThrow();
      } finally {
        await noRootsContext.teardown();
      }
    },
    30_000,
  );
