...
describe('server-transport-http / 14-tasks', () => {
  ...
  beforeAll(async () => {
    ...
    // NOTE: Using raw HTTP client because connector callTool with task metadata requires raw protocol-level control
  ...
  describe('task polling', () => {
    it('should poll task status via tasks/get [TASK-002]', async () => {
       ...
       */
      // NOTE: Using raw HTTP client because task was created in the same raw session
  ...
  describe('task listing', () => {
    it('should list tasks via tasks/list [TASK-004]', async () => {
       ...
       */
      // NOTE: Using raw HTTP session because tasks were created in this session.
      // Using the connector would query a different session that has no tasks.
      const result = (await rawSession.send('tasks/list', {})) as TaskListResult;
    ...
    it('should support pagination via cursor parameter [TASK-004]', async () => {
      ...
      expect(result.tasks).toBeDefined();
      expect(Array.isArray(result.tasks)).toBe(true);
      // the raw session has created multiple tasks at this point
      expect(result.tasks.length).toBeGreaterThanOrEqual(2);
    });
  });

  describe('task status notifications', () => {
    it('should emit notifications/tasks/status when task completes [TASK-008]', async () => {
      // SPEC ALIGNMENT: PASS
      /**
