...
describe('client cancels in-flight request', () => {
  serverHttpTest(
    'should complete faster than full duration when cancelled mid-flight [CANCEL-001]',
    async ({ mcpEndpoint }) => {
      // SPEC ALIGNMENT: PASS
      /**
      ...
      try {
        ...
        await new Promise<void>((resolve) =>
          setTimeout(resolve, CANCEL_DELAY_MS),
        );
...
describe('cancel already-completed request', () => {
  serverHttpTest(
    'should ignore cancellation notification for a completed request without errors [CANCEL-003]',
    async ({ mcpEndpoint }) => {
      // SPEC ALIGNMENT: PASS
      /**
...
describe('race condition: response after cancel', () => {
  serverHttpTest(
    'should handle cancel near completion without crashing [CANCEL-004]',
    async ({ mcpEndpoint }) => {
      // SPEC ALIGNMENT: PASS
      /**
      ...
      try {
        ...
        expect(healthCheckResult.content[0]?.text).toBe(
          'post-race-health-check',
        );
      } finally {
        await rawSession.close();
      }
