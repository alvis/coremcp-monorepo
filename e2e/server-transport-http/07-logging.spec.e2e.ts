...
serverHttpTest(
  'should set log level to debug [LOGGING-001]',
  async ({ connector }) => {
    // SPEC ALIGNMENT: PASS
    /**
...
serverHttpTest(
  'should set log level to info [LOGGING-001]',
  async ({ connector }) => {
    // SPEC ALIGNMENT: PASS
    /**
...
serverHttpTest(
  'should set log level to warning [LOGGING-001]',
  async ({ connector }) => {
    // SPEC ALIGNMENT: PASS
    /**
...
serverHttpTest(
  'should set log level to error [LOGGING-001]',
  async ({ connector }) => {
    // SPEC ALIGNMENT: PASS
    /**
...
serverHttpTest(
  'should trigger server-initiated log notification [LOGGING-002]',
  async ({ connector }) => {
    // SPEC ALIGNMENT: PASS
    /**
     ...
     */
    const result = await connector.callTool('trigger-log', {
      level: 'info',
...
serverHttpTest(
  'should trigger log notification at different levels [LOGGING-002]',
  async ({ connector }) => {
    // SPEC ALIGNMENT: PASS
    /**
...
describe('log level filtering [LOGGING-001]', () => {
  serverHttpTest(
    'should receive error-level log after setting level to warning [LOGGING-001]',
    async () => {
      // SPEC ALIGNMENT: PASS
      /**
      ...
      try {
        ...
        await notifyContext.connector.setLogLevel('warning');

        // trigger an info-level log (below warning threshold)
        await notifyContext.connector.callTool('trigger-log', {
          level: 'info',
        ...
        const infoNotifications = logNotifications.filter(
          (n) => (n as LoggingMessageNotification).params.level === 'info',
        );
...
describe('log notification payload [LOGGING-002]', () => {
  serverHttpTest(
    'should include level, data, and logger fields in notification [LOGGING-002]',
    async () => {
      // SPEC ALIGNMENT: PASS
      /**
...
describe('all 8 standard log levels [LOGGING-003]', () => {
  serverHttpTest(
    'should emit notifications for all 8 RFC 5424 log levels [LOGGING-003]',
    async () => {
      // SPEC ALIGNMENT: PASS
      /**
      ...
      const levelsContext = await createServerHttpClientContext({
        port: inject('coreMcpPort'),
        onNotification: async (notification) => {
          notifications.push(notification);
        },
      ...
      try {
        await levelsContext.connector.connect();

        // set log level to debug so all levels pass through
        await levelsContext.connector.setLogLevel('debug');

        for (const level of allLevels) {
          await levelsContext.connector.callTool('trigger-log', {
            level,
