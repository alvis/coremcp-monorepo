...
serverHttpTest(
  'should respond to client-initiated ping [PING-001]',
  async ({ connector }) => {
    // SPEC ALIGNMENT: PASS
    /**
...
serverHttpTest(
  'should handle server-initiated ping via trigger-ping [PING-002]',
  async ({ connector }) => {
    // SPEC ALIGNMENT: PASS
    /**
    ...
    const result = await connector.callTool('trigger-ping');
...
serverHttpTest(
  'should respond to ping before notifications/initialized is sent [PING-001]',
  async ({ mcpEndpoint }) => {
    ...
    const pingResponse = await fetch(mcpEndpoint, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Accept': 'application/json, text/event-stream',
    ...
    const rawText = await pingResponse.text();

    // parse based on Content-Type: SSE wraps JSON in "data: <json>\n\n"
    const pingBody = contentType.includes('text/event-stream')
      ? JSON.parse(
          rawText
            .split('\n')
