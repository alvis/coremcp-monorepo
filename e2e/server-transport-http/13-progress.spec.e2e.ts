...
describe('progress with known total', () => {
  serverHttpTest(
    'should send progress notifications during long operations [PROGRESS-001]',
    async () => {
      // SPEC ALIGNMENT: PASS
      /**
      ...
      const progressContext = await createServerHttpClientContext({
      ...
      await progressContext.connector.connect();
      ...
      try {
        ...
        const result = await progressContext.connector.callTool(
          'slow-operation',
          {
            duration: 2,
          },
        ...
        await progressContext.teardown();
      }
    },
    30_000,
  );
