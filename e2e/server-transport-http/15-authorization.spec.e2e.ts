...
describe('server-transport-http / 15-authorization', () => {
  ...
  beforeAll(async () => {
    const authServerPort = await startAuthServer();
    state.oauthBaseUrl = `http://localhost:${authServerPort}`;
    const { process: proc, port } = await spawnAuthHttpTestServer({ authServerPort });
  ...
  describe('client information metadata document', () => {
    it('should return client metadata after dynamic registration [AUTH-003]', async () => {
      ...
      const regResponse = await fetch(`${state.oauthBaseUrl}/register`, {
        method: 'POST',
  ...
  describe('re-authorization cascade', () => {
    it('should require full re-authorization when expired code and invalid refresh token are used [AUTH-007]', async () => {
      ...
      const expiredCodeResponse = await fetch(`${state.oauthBaseUrl}/token`, {
        method: 'POST',
        ...
        body: new URLSearchParams({
          ...
          code_verifier:
            'some-verifier-that-is-at-least-43-characters-long-for-pkce',
        }).toString(),
      });

      expect(expiredCodeResponse.status).toBe(400);
      const codeError =
        (await expiredCodeResponse.json()) as OAuthErrorResponse;
      expect(codeError.error).toBe('invalid_grant');

      // step 2: attempt refresh with an invalid/expired refresh token
      const refreshResponse = await fetch(`${state.oauthBaseUrl}/token`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: new URLSearchParams({
  ...
  describe('OAuth metadata discovery', () => {
    ...
    it('should serve authorization server metadata at well-known endpoint [AUTH-002]', async () => {
       ...
       */
      const response = await fetch(
        `${state.oauthBaseUrl}/.well-known/oauth-authorization-server`,
      );
  ...
  describe('dynamic client registration', () => {
    it('should register a new client dynamically [AUTH-003]', async () => {
       ...
       * @see https://github.com/modelcontextprotocol/typescript-sdk/blob/ccb78f20438a853f0205b8ed7f642af7780208e6/packages/client/src/client/auth.ts#L1510-L1540 (registerClient)
       */
      const response = await fetch(`${state.oauthBaseUrl}/register`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  describe('OAuth 2.1 authorization code + PKCE flow', () => {
    it('should issue authorization code via authorize endpoint with PKCE [AUTH-004]', async () => {
      ...
      const codeVerifier =
        'test-code-verifier-that-is-at-least-43-characters-long-for-pkce-spec';
      ...
      const authorizeUrl = new URL(`${state.oauthBaseUrl}/authorize`);
      ...
      const tokenResponse = await fetch(`${state.oauthBaseUrl}/token`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: new URLSearchParams({
          grant_type: 'authorization_code',
    ...
    it('should reject invalid code_verifier during token exchange [AUTH-004]', async () => {
      ...
      const codeVerifier =
        'valid-code-verifier-that-is-at-least-43-characters-long-for-pkce';
      const encoder = new TextEncoder();
      ...
      const tokenResponse = await fetch(`${state.oauthBaseUrl}/token`, {
        ...
        body: new URLSearchParams({
          ...
          client_id: registration.client_id,
          code_verifier:
            'wrong-verifier-does-not-match-the-challenge-at-all-and-is-long',
        }).toString(),
      });
  ...
  describe('token refresh', () => {
    it('should refresh an access token using refresh_token grant [AUTH-005]', async () => {
      ...
      const codeVerifier =
        'refresh-test-code-verifier-that-is-at-least-43-characters-long';
      const encoder = new TextEncoder();
      const hashBuffer = await crypto.subtle.digest(
        'SHA-256',
        encoder.encode(codeVerifier),
    ...
    it('should reject reuse of rotated refresh token [AUTH-005]', async () => {
      ...
      const codeVerifier =
        'rotation-test-verifier-that-is-at-least-43-characters-long-here';
      const encoder = new TextEncoder();
      const hashBuffer = await crypto.subtle.digest(
        'SHA-256',
        encoder.encode(codeVerifier),
      ...
      await fetch(`${state.oauthBaseUrl}/token`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: new URLSearchParams({
          grant_type: 'refresh_token',
      ...
      const reuseResponse = await fetch(`${state.oauthBaseUrl}/token`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: new URLSearchParams({
          grant_type: 'refresh_token',
  ...
  describe('scope enforcement', () => {
    it('should return 403 with insufficient_scope when token lacks required scope [AUTH-006]', async () => {
      ...
      const codeVerifier =
        'scope-enforcement-verifier-that-is-at-least-43-characters-long-here';
      const encoder = new TextEncoder();
      const hashBuffer = await crypto.subtle.digest(
        'SHA-256',
        encoder.encode(codeVerifier),
    ...
    it('should return 403 when MCP request uses token without required mcp scope [AUTH-006]', async () => {
      ...
      const codeVerifier =
        'insufficient-scope-verifier-that-is-at-least-43-characters-long-x';
      const encoder = new TextEncoder();
      const hashBuffer = await crypto.subtle.digest(
        'SHA-256',
        encoder.encode(codeVerifier),
  ...
  describe('full PKCE OAuth flow through to MCP call', () => {
    it('should complete PKCE flow and use token to call MCP endpoint [AUTH-004]', async () => {
      ...
      const codeVerifier =
        'full-flow-test-code-verifier-that-is-at-least-43-characters-long-here';
      const encoder = new TextEncoder();
      const hashBuffer = await crypto.subtle.digest(
        'SHA-256',
        encoder.encode(codeVerifier),
