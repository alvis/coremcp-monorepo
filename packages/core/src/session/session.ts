...
/** rfc 5424 severity ordering for mcp log levels (lower value = more severe) */
...
export class Session {
  ...
  /** minimum log level threshold set by client via logging/setLevel (runtime-only, not persisted) */
  #logLevel: McpLogLevel | null = null;
  /** runtime event listeners for live notification forwarding */
  #listeners = new Set<EventHook>();
  ...
  /** gets the optional usage instructions for the server */
  ...
  /** gets the minimum log level threshold set by client */
  public get logLevel(): McpLogLevel | null {
    return this.#logLevel;
  }

  /**
   * sets the minimum log level threshold for notifications/message filtering
   * @param level mcp log level threshold
   */
  public set logLevel(level: McpLogLevel | null) {
    this.#logLevel = level;
  }

  /**
   * registers a runtime listener that fires on every new event
   * @param callback function invoked with each session event
   * @returns unsubscribe function that removes the listener
   */
  public addListener(callback: EventHook): () => void {
  ...
  /**
   * sends a log notification to the client, filtered by the configured log level threshold
   ...
   * @param options.channelId when provided, scopes the notification to a specific channel
   */
  public async notify(
