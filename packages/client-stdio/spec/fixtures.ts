...
export class FakeProcess extends EventEmitter {
  ...
  constructor(command?: string, args?: readonly string[]) {
    ...
    if (command !== undefined || args !== undefined) {
      this.initializeDrivingBehavior(args);
    }
  }
  ...
  /**
   * initializes self-driving mock behavior based on command arguments
   * @param args command-line arguments that control mock behavior
   */
  private initializeDrivingBehavior(args: readonly string[] | undefined): void {
    const arr = args ?? [];
  ...
  /**
   * sets up auto-responder for incoming requests
   * @param args command-line arguments that control mock behavior
   */
  private setupAutoResponder(args: readonly string[]): void {
    ...
    if (has('--exit-on-sigterm')) {
      ...
      }) as Partial<typeof this.kill> as typeof this.kill;
    }
    ...
    if (has('--stdin-close-error')) {
      ...
      }) as Partial<typeof this.stdin.end> as typeof this.stdin.end;
    }
