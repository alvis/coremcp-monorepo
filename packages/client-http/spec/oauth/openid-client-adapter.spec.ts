...
describe('fn:discoverFromChallenge', () => {
  ...
  it('should support path-aware protected resource metadata challenges', async () => {
    const challenge =
    ...
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        new Response(JSON.stringify(resourceMetadata), {
          status: 200,
          headers: { 'content-type': 'application/json' },
        }),
      )
      .mockResolvedValueOnce(
        new Response(JSON.stringify(MOCK_AUTH_SERVER_METADATA), {
          status: 200,
          headers: { 'content-type': 'application/json' },
        }),
      );
    ...
    const result = await discoverFromChallenge(challenge);
  ...
  it('should fall back to OpenID metadata when OAuth AS metadata endpoint is unavailable', async () => {
    const resourceMetadata = {
      ...MOCK_RESOURCE_METADATA,
    ...
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        new Response(JSON.stringify(authServerMetadata), {
          status: 200,
          headers: { 'content-type': 'application/json' },
        }),
      );
    ...
    expect(result.authServerMetadata).toEqual(authServerMetadata);
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
