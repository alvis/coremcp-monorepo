...
describe('fn:fetchResourceMetadata', () => {
  ...
  it('should use path-aware protected resource metadata URL for scoped resources', async () => {
  ...
  it('should fall back to legacy root metadata endpoint when scoped metadata is unavailable', async () => {
    const resourceUrl = 'https://mcp.example.com/mcp';
    const metadata: ProtectedResourceMetadata = {
      resource: resourceUrl,
      authorization_servers: ['https://auth.example.com'],
