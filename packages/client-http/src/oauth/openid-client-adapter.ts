...
import { fetchResourceMetadataFromUrl } from './resource-metadata';
...
function buildWellKnownUrl(
  ...
  return {
    primary: `${url.origin}${suffix}${pathname}`,
    fallback: pathname ? `${url.origin}${suffix}` : undefined,
  };
}
...
async function discoverAuthorizationServerMetadata(
  ...
  const attempts = [
    buildWellKnownUrl(issuer, AUTH_SERVER_METADATA_PATH),
    buildWellKnownUrl(issuer, OPENID_CONFIGURATION_PATH),
  ];

  for (const attempt of attempts) {
    try {
      return await fetchAuthorizationServerMetadata(attempt.primary);
    } catch (error) {
      if (
        error instanceof ExternalError &&
        attempt.fallback &&
        /Failed to fetch authorization server metadata: 4\d\d /.test(
          error.message,
        )
      ) {
        try {
          return await fetchAuthorizationServerMetadata(attempt.fallback);
        } catch (fallbackError) {
          if (fallbackError instanceof ExternalError) {
            continue;
          }
...
export async function discoverFromChallenge(
  ...
  const resourceMetadata = await fetchResourceMetadataFromUrl(
    challengeInfo.resourceMetadata,
  );
