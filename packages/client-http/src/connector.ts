...
export class HttpMcpConnector extends McpConnector {
  ...
  #sessionId?: string;
  /** tracked stream handler promises for coordinated shutdown */
  ...
  protected async [disconnect](): Promise<void> {
    // 1. abort SSE streams first to stop reconnection attempts
    if (this.#abortController) {
      this.#abortController.abort();
      this.#abortController = undefined;
    }

    // 2. wait for all stream handlers to settle
    await Promise.allSettled(this.#streamHandlers);
    this.#streamHandlers = [];

    // 3. send session termination after streams fully stopped
    ...
    // 4. clear session ID after termination
  ...
  async #processHttpResponse(response: Response): Promise<void> {
    ...
    if (contentType?.includes('application/json')) {
      ...
      this.#streamHandlers.push(streamPromise);
    }
  }
  ...
  async #onInitialized(): Promise<void> {
    ...
    const streamPromise = handleStream({
      getStream: async ({ lastEventId }) => this.connectToStream(lastEventId),
      onMessage: this[onMessage].bind(this),
      abortSignal: this.#abortController?.signal,
    });
    this.#streamHandlers.push(streamPromise);
