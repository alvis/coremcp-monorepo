...
/**
 * processes forward result and stores token mappings if successful.
...
/**
 * exchanges authorization code for tokens via external AS.
...
/**
 * exchanges refresh token for new tokens via external AS.
...
async function validateAndExtractCredentials(
  // eslint-disable-next-line @typescript-eslint/naming-convention -- OAuth wire format uses snake_case parameters per RFC 6749
...
/**
 * proxies token request to external AS, dispatching the appropriate grant handler.
 * @param request fastify request with token body
 * @param reply fastify reply object
 * @param config OAuth proxy configuration
 * @param storage proxy storage adapter
