...
export class McpStdioServerTransport extends ServerTransport {
  ...
  /**
   * handles a single incoming line from stdin
   * @param line the raw line received from stdin
   * @param context the connection context for this transport
   */
  async #handleLine(line: string, context: ConnectionContext): Promise<void> {
    ...
    if (message) {
      ...
      // respond with JSON-RPC parse error per spec (id: null when request is unparseable)
      await this.#send({
        jsonrpc: JSONRPC_VERSION,
  ...
  public async [start](): Promise<void> {
    ...
    this.#rl.on('line', async (line) => this.#handleLine(line, context));
